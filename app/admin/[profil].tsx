 ...
 *
 * LA comparaison qui fait la validation — le selfie À CÔTÉ de la pièce
 * d'identité — est en tête, en grand. Le reste suit. Un écran qui oblige à
 * faire défiler entre les deux photos ne sert à rien : on ne compare pas deux
 * visages de mémoire.
 *
...
const MOTIFS = ['motifIllisible', 'motifExpiree', 'motifNeCorrespondPas', 'motifAutre'] as const;
    ...
    <View className="flex-1 bg-bg">
      ...
      >
        ...
        {statut === 'chargement' ? (
          ...
          <>
            {/* LA comparaison. Côte à côte, en tête, en grand. */}
            ...
            >
              <View className="flex-1">
                <Vignette
                  titre={t('admin.pieceIdentite')}
                  uri={urls.piece_identite ?? null}
                  grande
                />
              </View>
              <View className="flex-1">
                <Vignette titre={t('admin.selfie')} uri={urls.selfie ?? null} grande />
              </View>
            </View>
            ...
            {/* Chaque pièce, avec sa décision. */}
            {(['piece_identite', 'permis', 'carte_grise', 'selfie'] as TypeDocument[]).map(
              (type) => (
                <PieceAdmin
                  key={type}
